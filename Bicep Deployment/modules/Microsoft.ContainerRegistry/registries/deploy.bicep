...
param diagnosticWorkspaceIdMetric string = ''
...
param diagnosticSettingsMetric string = '${name}-diagnosticSettings-metrics'
...
var diagnosticsLogsSpecified = [for category in filter(diagnosticLogCategoriesToEnable, item => item != 'audit'): {
  category: category
  enabled: true
  retentionPolicy: {
    enabled: true
...
var diagnosticsLogs = contains(diagnosticLogCategoriesToEnable, 'audit') ? [
  {
    categoryGroup: 'audit'
    enabled: true
    retentionPolicy: {
...
resource registry_diagnosticSettingMetrtic 'Microsoft.Insights/diagnosticsettings@2021-05-01-preview' = if ((!empty(diagnosticStorageAccountId)) || (!empty(diagnosticWorkspaceIdMetric)) || (!empty(diagnosticEventHubAuthorizationRuleId)) || (!empty(diagnosticEventHubName))) {
  name: diagnosticSettingsMetric
  properties: {
    storageAccountId: !empty(diagnosticStorageAccountId) ? diagnosticStorageAccountId : null
    workspaceId: !empty(diagnosticWorkspaceIdMetric) ? diagnosticWorkspaceIdMetric : null
    eventHubAuthorizationRuleId: !empty(diagnosticEventHubAuthorizationRuleId) ? diagnosticEventHubAuthorizationRuleId : null
    eventHubName: !empty(diagnosticEventHubName) ? diagnosticEventHubName : null
    metrics: diagnosticsMetrics
    // logs: diagnosticsLogs
...
module registry_privateEndpoints '../../Networking/privateEndpoints/Privateendpoint.bicep' = [for (privateEndpoint, index) in privateEndpoints: {
