...
@description('Optional. Resource ID of the diagnostic log analytics workspace.')
param diagnosticWorkspaceIdDFS string = ''
...
// param diagnosticMetricsToEnable_DFS array = [
//   'ScanResults'
// ]
...
resource stgdefenderforstgenabled 'Microsoft.Security/DefenderForStorageSettings@2022-12-01-preview' = {
  name: 'current'
  scope: storageAccount
  properties: {
    isEnabled: true
    workspaceid: diagnosticWorkspaceIdDFS
    malwareScanning: {
      onUpload: {
        isEnabled: true
        capGBPerMonth: 5000
    ...
    scanResultsEventGridTopicResourceId: !empty(scanResultsEventGridTopicResourceId) ? scanResultsEventGridTopicResourceId : null
...
resource storageAccount_diagnosticSettings_DFS 'Microsoft.Insights/diagnosticSettings@2021-05-01-preview' = if ((!empty(diagnosticStorageAccountId)) || (!empty(diagnosticWorkspaceIdDFS)) || (!empty(diagnosticEventHubAuthorizationRuleId)) || (!empty(diagnosticEventHubName))) {
  name: diagnosticSettingsName
  properties: {
    storageAccountId: !empty(diagnosticStorageAccountId) ? diagnosticStorageAccountId : null
    workspaceId: !empty(diagnosticWorkspaceIdDFS) ? diagnosticWorkspaceIdDFS : null
    eventHubAuthorizationRuleId: !empty(diagnosticEventHubAuthorizationRuleId) ? diagnosticEventHubAuthorizationRuleId : null
    eventHubName: !empty(diagnosticEventHubName) ? diagnosticEventHubName : null
    logs: [
        {
  ...
  scope: stgdefenderforstgenabled
}
...
module storageAccount_privateEndpoints '../../Networking/privateEndpoints/Privateendpoint.bicep' = [for (privateEndpoint, index) in privateEndpoints: {
...
output containerid string = storageAccount_blobServices.outputs.resourceId

output queuename array = storageAccount_queueServices.outputs.queuename
