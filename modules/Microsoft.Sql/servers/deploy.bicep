...
resource sqlauditing 'Microsoft.Sql/servers/auditingSettings@2021-11-01' = {
  parent: server
  name: 'default'
  properties: {
...
resource sqlauditingdevopsaudit 'Microsoft.Sql/servers/devOpsAuditingSettings@2022-08-01-preview' = {
  parent: server
  name: 'Default'
  properties: {
    isAzureMonitorTargetEnabled: true
...
resource sqlauditingextended 'Microsoft.Sql/servers/extendedAuditingSettings@2022-08-01-preview' = {
  parent: server
  name: 'default'
  properties: {
...
module server_databases 'databases/db.bicep' = [for (database, index) in databases: {
  ...
  params: {
    ...
    skuName: contains(database, 'skuName') ? database.skuName : 'GP_S_Gen5'
    ...
    diagnosticWorkspaceIdaudit : contains(database, 'diagnosticWorkspaceIdAudit') ? database.diagnosticWorkspaceIdaudit : ''
...
module server_privateEndpoints '../../Networking/privateEndpoints/Privateendpoint.bicep' = [for (privateEndpoint, index) in privateEndpoints: {
...
output databaseresourceid string = server_databases[0].outputs.databaseresourceId
