...
// module virtualNetwork_subnets 'subnets/subnet.bicep' = [for (subnet, index) in subnets: {
...
module virtualNetwork_peering_local 'virtualNetworkPeerings/vnetpeer.bicep' = [for (peering, index) in virtualNetworkPeerings: {
...
module virtualNetwork_peering_remote 'virtualNetworkPeerings/vnetpeer.bicep' = [for (peering, index) in virtualNetworkPeerings: if (contains(peering, 'remotePeeringEnabled') ? peering.remotePeeringEnabled == true : false) {
  ...
  params: {
    localVnetName: peering.remoteVirtualNetworkId
